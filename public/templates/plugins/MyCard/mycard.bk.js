...
// setup
    var viewer = $(_options.renderTo).find('.viewer'),
        ...
        overlay = $('<div class="overlay"><i class="loading fa fa-refresh fa-spin text-blue"></i><div class="centered">'
                    +'<div class="control">'
                        +'<span action="'+_options.namespace+'.printItem" role="print" class="control-action text-blue"><i class="fa fa-print"></i></span>'
                        +'<span action="'+_options.namespace+'.downloadImage" role="run" class="control-action text-blue"><i class="fa fa-download"></i></span>'
                        +'<span action="'+_options.namespace+'.downloadImage" role="trying" class="control-action text-orange"><i class="fa fa-refresh"></i></span>'
                        +'<span action="'+_options.namespace+'.removeItem" role="remove" class="control-action text-red"><i class="fa fa-times"></i></span></div><!--control--></div><!--centered--></div><!--overlay-->'),
        template = $('<div id="'+_options.namespace+'" class="viewer"><p class="hint">Empty Card</p><!--hint--></div><!--viewer-->');

    if (!viewer.length) {
        $(_options.renderTo).prepend(template);
        viewer = $(_options.renderTo).find('.viewer');
    };

    if (!hint.length) {
        if (_options.hint)
            template.find('.hint').html(_options.hint);
...
// function
    ...
    function download_image(item_id) {
        ...
        var zip = new JSZip(),
            ...
            zip_folder = zip.folder('MyCard-'+_options.namespace),
        ...
        zip.generateAsync({type:'blob'}).then(function(content) {
            saveAs(content, 'MyCard-'+_options.namespace + '_' + mycard.attr('id') + '.zip'); // mycard_16050095.zip
        });

        // after download session
        if (_options.afterDownload)
            window[_options.afterDownload](item);
    }
    ...
    function download_all() {
        ...
        zip.generateAsync({type:'blob'}).then(function(content) {
            saveAs(content, 'MyCard-'+_options.namespace+'.zip'); // mycard.zip
        });
    }
    ...
    function print_item(item_id) {
        ...
        setTimeout(function(){
            ...
            // after print session
            if (_options.afterPrint)
            	window[_options.afterPrint](item);
        },100);
    }
    ...
    function print_all() {
        ...
        setTimeout(function(){
            ...
            if (_options.afterPrint) {
                $.each(items, function(index) {
                    window[_options.afterPrint](items[index]);
                })
            }
        },100);
    }
    ...
    function to_card(o) {
    	if (_options.toCard)
    	    return window[_options.toCard](o);
    	else
    		alert('Error: _options.toCard function need to set');
    }
